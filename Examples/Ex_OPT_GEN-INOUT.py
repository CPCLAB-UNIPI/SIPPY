...
if mode == 'IC':
    ...
    # ARMA - ARARX - ARARMAX
    Id_ARMA = system_identification(Ytot, Usim, 'ARMA', IC='BIC', na_ord=[2, 2], nc_ord=[2, 2], delays=[11, 11], max_iterations=300)
    
    Id_ARARX = system_identification(Ytot, Usim, 'ARARX', IC='BIC', na_ord=[4, 4], nb_ord=[3, 3], nd_ord=[3, 3], delays=[11, 11], max_iterations=300)
    
    Id_ARARMAX = system_identification(Ytot, Usim, 'ARARMAX', IC='BIC', na_ord=[4, 4], nb_ord=[3, 3], nc_ord=[2, 2], nd_ord=[3, 3], delays=[11, 11], max_iterations=300)
     
    # OE - BJ - GEN
    Id_OE = system_identification(Ytot, Usim, 'OE', IC='BIC', nb_ord=[3, 3], nf_ord=[4, 4], delays=[11, 11], max_iterations=300)
    #
    Id_BJ = system_identification(Ytot, Usim, 'BJ', IC='BIC', nb_ord=[3, 3], nc_ord=[2, 2], nd_ord=[3, 3], nf_ord=[4, 4], delays=[11, 11], max_iterations=300)
    # #
    Id_GEN = system_identification(Ytot, Usim, 'GEN', IC='BIC', na_ord=[2, 2], nb_ord=[3, 3], nc_ord=[2, 2], nd_ord=[3, 3], nf_ord=[4, 4], 
    ...
    na_ord = [2]; nb_ord = [[3]]; nc_ord = [2]; nd_ord = [3]; nf_ord = [4]; theta = [[11]]
    
    # ARMA - ARARX - ARARMAX
    Id_ARMA = system_identification(Ytot, Usim, 'ARMA', ARMA_orders = [na_ord, nc_ord, theta])
    # #
    Id_ARARX = system_identification(Ytot, Usim, 'ARARX', ARARX_orders = [na_ord, nb_ord, nd_ord, theta], max_iterations = 300)
    # #
    Id_ARARMAX = system_identification(Ytot, Usim, 'ARARMAX', ARARMAX_orders = [na_ord, nb_ord, nc_ord, nd_ord, theta], max_iterations = 300)

    # OE - BJ - GEN
    Id_OE = system_identification(Ytot, Usim, 'OE', OE_orders = [nb_ord, nf_ord, theta], max_iterations = 300)
    #
    Id_BJ = system_identification(Ytot, Usim, 'BJ', BJ_orders = [nb_ord, nc_ord, nd_ord, nf_ord, theta], max_iterations = 300)
    # #
    Id_GEN = system_identification(Ytot, Usim, 'GEN', GEN_orders = [na_ord, nb_ord, nc_ord, nd_ord, nf_ord, theta], max_iterations = 300)
#
...
plt.loglog(om,mag1), plt.loglog(om,mag2), plt.loglog(om,mag3), plt.loglog(om,mag4), plt.loglog(om,mag5), plt.loglog(om,mag6)
plt.xlabel("w"),plt.ylabel("Amplitude Ratio"), plt.title("Bode Plot G(iw)")
plt.subplot(2,1,2), plt.semilogx(om,fi), plt.grid()
plt.semilogx(om,fi1), plt.semilogx(om,fi2), plt.semilogx(om,fi3), plt.semilogx(om,fi4), plt.semilogx(om,fi5), plt.semilogx(om,fi6)
