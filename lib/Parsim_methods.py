...
def estimating_y(H_K,Uf,G_K,Yf,i,m,l):
    ...
    for j in range(1,i):
        y_tilde=y_tilde+np.dot(H_K[l*j:l*(j+1),:],Uf[m*(i-j):m*(i-j+1),:])+np.dot(G_K[l*j:l*(j+1),:],Yf[l*(i-j):l*(i-j+1),:])
    return y_tilde
...
def simulations_sequence(A_K,C,L,y,u,l,m,n,D_required):
    ...
    if D_required==True:
        ...
        for i in range(n_simulations):
            ...
            y_sim.append((SS_lsim_predictor_form(A_K,B_K,C,D,K,y,u,x0)[1]).reshape((1,L*l)))
            vect[i,0]=0.
    else:
        D=np.zeros((l,m))
...
def PARSIM_K(y,u,f=20,p=20,threshold=0.1,max_order=np.NaN,fixed_order=np.NaN,D_required=False,B_recalc=False):
    y=1.*np.atleast_2d(y)
    u=1.*np.atleast_2d(u)
    l,L=y.shape
    m=u[:,0].size
    if check_types(threshold,max_order,fixed_order,f,p)==False:
        ...
        Vn=0.5*np.trace(np.dot(np.dot(y_sim,vect)-y.reshape((L*l,1)),(np.dot(y_sim,vect)-y.reshape((L*l,1))).T))/L
        B_K=vect[0:n*m,:].reshape((n,m))
        if D_required==True:
            D=vect[n*m:n*m+l*m,:].reshape((l,m))
            K=vect[n*m+l*m:n*m+l*m+n*l,:].reshape((n,l))
...
def select_order_PARSIM_K(y,u,f=20,p=20,method='AIC',orders=[1,10],D_required=False,B_recalc=False):
    y=1.*np.atleast_2d(y)
    u=1.*np.atleast_2d(u)
    min_ord=min(orders)
    l,L=y.shape
...
def PARSIM_S(y,u,f=20,p=20,threshold=0.1,max_order=np.NaN,fixed_order=np.NaN,D_required=False):
    y=1.*np.atleast_2d(y)
    u=1.*np.atleast_2d(u)
    l,L=y.shape
    m=u[:,0].size
...
def PARSIM_P(y,u,f=20,p=20,threshold=0.1,max_order=np.NaN,fixed_order=np.NaN,D_required=False):
    y=1.*np.atleast_2d(y)
    u=1.*np.atleast_2d(u)
    l,L=y.shape
    m=u[:,0].size
