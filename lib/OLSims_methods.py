...
def forcing_A_stability(M,n,Ob,l,X_fd,N,u,f):
    ...
    if np.max(np.abs(np.linalg.eigvals(M[0:n,0:n])))>=1.:
        ...
        M[0:n,n::]=np.dot(X_fd[:,1:N]-np.dot(M[0:n,0:n],X_fd[:,0:N-1]),np.linalg.pinv(u[:,f:f+N-1]))
    res=X_fd[:,1:N]-np.dot(M[0:n,0:n],X_fd[:,0:N-1])-np.dot(M[0:n,n::],u[:,f:f+N-1])
...
def OLSims(y,u,f,weights='N4SID',threshold=0.1,max_order=np.NaN,fixed_order=np.NaN,D_required=False,A_stability=False):
    y=1.*np.atleast_2d(y)
    u=1.*np.atleast_2d(u)
    l,L=y.shape
    m=u[:,0].size
    if check_types(threshold,max_order,fixed_order,f)==False:
        ...
        Ob,X_fd,M,n,residuals=algorithm_1(y,u,l,m,f,N,U_n,S_n,V_n,W1,O_i,threshold,max_order,D_required)
        if A_stability==True:
...
def select_order_SIM(y,u,f=20,weights='N4SID',method='AIC',orders=[1,10],D_required=False,A_stability=False):
    y=1.*np.atleast_2d(y)
    u=1.*np.atleast_2d(u)
    min_ord=min(orders)
    l,L=y.shape
    ...
    if check_types(0.0,np.NaN,np.NaN,f)==False:
        ...
        for i in range(min_ord,max_ord):
            Ob,X_fd,M,n,residuals=algorithm_1(y,u,l,m,f,N,U_n,S_n,V_n,W1,O_i,0.0,i,D_required)
            if A_stability==True:
