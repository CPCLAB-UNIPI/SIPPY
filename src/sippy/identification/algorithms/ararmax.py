...
class ARARMAXAlgorithm(IdentificationAlgorithm):
    ...
    def validate_config(self, config):
        ...
        if not hasattr(config, 'na') or config.na is None:
            raise ValueError("ARARMAX algorithm requires 'na' parameter (AR order for y)")
        if not hasattr(config, 'nb') or config.nb is None:
            raise ValueError("ARARMAX algorithm requires 'nb' parameter (input polynomial order)")
        if not hasattr(config, 'nc') or config.nc is None:
            raise ValueError("ARARMAX algorithm requires 'nc' parameter (noise AR order)")
        if not hasattr(config, 'nd') or config.nd is None:
            raise ValueError("ARARMAX algorithm requires 'nd' parameter (noise MA order)")
        if not hasattr(config, 'nf') or config.nf is None:
            raise ValueError("ARARMAX algorithm requires 'nf' parameter (input TF order)")
    ...
    def identify(self, y: Optional[np.ndarray] = None, u: Optional[np.ndarray] = None,
        ...
        if iddata is not None:
            ...
            data_df.update(pd.DataFrame({
                f'y{i+1}': y[:, i] if len(y.shape) > 1 else y
                for i in range(len(y[0]) if len(y.shape) > 1 else 1)
            }))
            data = IDData(data=data_df, 
                         inputs=[f'u{i+1}' for i in range(len(u[0]) if len(u.shape) > 1 else 1)],
                         outputs=[f'y{i+1}' for i in range(len(y[0]) if len(y.shape) > 1 else 1)])
            config = kwargs.get('config')
        else:
        ...
        if n_samples <= max_order + 10:  # Need enough data for estimation
            raise ValueError(f"Insufficient data: need at least {max_order + 10} samples, got {n_samples}")
        ...
        if HAROLD_AVAILABLE:
            ...
            try:
                ...
                model = StateSpaceModel(
                    A=harold_ss.A, B=harold_ss.B, C=harold_ss.C, D=harold_ss.D,
                ...
                warnings.warn(f"Harold state space realization failed: {e}. Using fallback.")
        ...
        try:
            ...
            B_aug = np.zeros((n_states, len(u_values[0]) if len(u_values.shape) > 1 else 1))
            C_aug = np.zeros((len(y_values[0]) if len(y_values.shape) > 1 else 1, n_states))
            D_aug = np.zeros((len(y_values[0]) if len(y_values.shape) > 1 else 1, 
            ...
            if n_noise_ar > 0:
                ...
                if n_noise_ar > 0:
                    for i in range(n_noise_ar):
                        if i < len(theta):
                            A_aug[ar_end - 1, ar_start + i] = -theta[max(na) + max(nb) + i] if (max(na) + max(nb) + i) < len(theta) else 0
            ...
            from ..base import StateSpaceModel
            model = StateSpaceModel(
                A=A_aug, B=B_aug, C=C_aug, D=D_aug,
            ...
            from ..base import StateSpaceModel
            model = StateSpaceModel(
                A=A_fallback, B=B_fallback, C=C_fallback, D=D_fallback,
    ...
    def _build_regression_matrices_ararmax(self, u, y, na, nb, nc, nd, nf, nk):
        ...
        n_params_ar = max(na) * n_outputs
        n_params_input = max(nb) * n_inputs
        n_params_noise_ar = max(nc)
        n_params_noise_ma = max(nd)
        n_params_total = n_params_ar + n_params_input + n_params_noise_ar + n_params_noise_ma
        
        # Initialize regression matrices
        phi = np.zeros((n_samples - max(max(na), max(nb) + max(nk), max(nc), max(nd), max(nf)), n_params_total))
        target = np.zeros(n_samples - max(max(na), max(nb) + max(nk), max(nc), max(nd), max(nf)))
        ...
        for k in range(max_order, n_samples):
            ...
            for i in range(1, min(max(na) + 1, k + 1)):
                for j in range(n_outputs):
                    if row_idx < phi.shape[1]:
                        phi[k - max_order, row_idx] = float(y[k - i][j] if n_outputs > 1 else y[k - i][0])
                        row_idx += 1
                    else:
                        break
            
            # Input terms - handle SISO properly
            for i in range(max(nk), min(max(nb) + max(nk) + 1, k + 1)):
                for j in range(n_inputs):
                    if row_idx < phi.shape[1]:
                        phi[k - max_order, row_idx] = float(u[k - i][j] if n_inputs > 1 else u[k - i][0])
                        row_idx += 1
                    else:
                        break
            
            # Noise AR terms (approximated with residuals)
            for i in range(1, min(max(nc) + 1, k + 1)):
                # Use approximate residual y[k] - predicted y[k]
                if k >= max(na) + max(nb):
                    pred = sum((y[k - j - 1][0] if n_outputs > 1 else y[k - j - 1]) * (0.1 if j < max(na) else 0) for j in range(max(max(na), 1)))
                    resid = (y[k][0] if n_outputs > 1 else y[k]) - pred
                else:
                    resid = (y[k][0] if n_outputs > 1 else y[k]) * 0.1  # Initial approximation
                if row_idx < phi.shape[1]:
            ...
            for i in range(1, min(max(nd) + 1, k + 1)):
                if k >= i:
                    # Use difference of residuals
                    if k >= max(na) + max(nb):
                        pred1 = sum((y[k - j - 1][0] if n_outputs > 1 else y[k - j - 1]) * (0.1 if j < max(na) else 0) for j in range(max(max(na), 1)))
                        pred2 = sum((y[k-i - j - 1][0] if n_outputs > 1 else y[k-i - j - 1]) * (0.1 if j < max(na) else 0) for j in range(max(max(na), 1)))
                        resid_diff = ((y[k][0] if n_outputs > 1 else y[k]) - pred1) - ((y[k-i][0] if n_outputs > 1 else y[k-i]) - pred2)
                    else:
                        resid_diff = ((y[k][0] if n_outputs > 1 else y[k]) - (y[k-i][0] if n_outputs > 1 else y[k-i])) * 0.1
                    if row_idx < phi.shape[1]:
    ...
    def _extract_state_space_matrices_ararmax(self, theta, na, nb, nc, nd, nf, nk, sample_time):
    ...
    def _create_fallback_armax_model(self, u, y, config):
        ...
        arx_iddata = IDData(
            data=pd.DataFrame({
                **{f'u{i+1}': u_values[:, i] if len(u_values.shape) > 1 else u_values 
                   for i in range(len(u_values[0]) if len(u_values.shape) > 1 else 1)},
                **{f'y{i+1}': y_values[:, i] if len(y_values.shape) > 1 else y_values 
                   for i in range(len(y_values[0]) if len(y_values.shape) > 1 else 1)}
            }),
            inputs=[f'u{i+1}' for i in range(len(u_values[0]) if len(u_values.shape) > 1 else 1)],
            outputs=[f'y{i+1}' for i in range(len(y_values[0]) if len(y_values.shape) > 1 else 1)],
