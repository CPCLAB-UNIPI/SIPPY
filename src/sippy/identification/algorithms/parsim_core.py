...
class ParsimCoreAlgorithm:
    ...
    @staticmethod
    def parsim_k(y, u, f=20, p=20, threshold=0.1, max_order=np.nan,
                 fixed_order=np.nan, D_required=False, B_recalc=False):
        """
        ...
        if not check_types(threshold, max_order, fixed_order, f, p):
            return np.array([[0.0]]), np.array([[0.0]]), np.array([[0.0]]), \
                   ...
                   np.array([[0.0]]), np.array([[0.0]]), np.inf
        ...
        U_n, S_n, V_n, W1, O_i = SubspaceCoreAlgorithm.svd_weighted(y, u, f, l_, 'N4SID')
        ...
        if B_recalc:
            try:
                ...
                X_states = []
                for t in range(L - 1):
                    x_next = np.dot(A_K, y[:, t].reshape((l_, 1))) + np.dot(B_K, u[:, t].reshape((m, 1)))
    ...
    @staticmethod
    def parsim_s(y, u, f=20, p=20, threshold=0.1, max_order=np.nan,
    ...
    def parsim_p(y, u, f=20, p=20, threshold=0.1, max_order=np.nan,
        ...
        return ParsimCoreAlgorithm.parsim_s(y, u, f, p, threshold, max_order,
                                           fixed_order, D_required)
