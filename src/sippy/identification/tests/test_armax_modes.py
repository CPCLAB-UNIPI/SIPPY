...
from ..algorithms.armax_modes import get_armax_handler, ILLSHandler, OPTHandler, RLLSHandler
    ...
    def test_algorithm_info_storage(self, sample_data, config_params):
        ...
        try:
            ...
            if hasattr(model, '_identification_info'):
                ...
                possible_keys = ['iterations', 'final_variance', 'converged', 'optimal_success']
        ...
        for mode in ['ILLS', 'RLLS']:  # Skip OPT as it's most complex
            ...
            for i in range(5):
                try:
                    ...
                    total_elements = (model.A.size + model.B.size + 
                                    model.C.size + model.D.size)
