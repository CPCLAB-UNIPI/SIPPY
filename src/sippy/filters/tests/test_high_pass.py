...
from sippy.filters.utils import create_test_data
...
class TestHighPassFilter:
    ...
    def test_high_pass_basic(self):
        ...
        assert filtered_rms < original_rms * 0.5, f"High-pass filter didn't sufficiently reduce low-frequency components"
    ...
    def test_high_pass_parameters(self):
        ...
        for cutoff in cutoffs:
            ...
            filtered_rms = result['signal1'].std()
            print(f"Cutoff {cutoff} Hz: original RMS={signal_rms:.4f}, filtered RMS={filtered_rms:.4f}")
    ...
    def test_edge_cases(self):
        ...
        data = pd.DataFrame({'x': [1, 2, 3]}, index=pd.date_range('2023-01-01', periods=3, freq='1D'))
...
class TestDifferenceFilter:
    ...
    def test_first_order_difference(self):
        ...
        assert result['y'].iloc[0] == 0  # First difference of first element should be zero
...
class TestZeroMeanFilter:
    ...
    def test_zero_mean_with_slices(self):
        ...
        assert result.iloc[bad_start:bad_end+1, 'signal1'].isna().all()
        assert result.iloc[bad_start:bad_end, 'signal2'].diff().isna().all()  # Bad slice in signal2
...
class TestNoneFilter:
    ...
    def test_none_with_slices(self):
        ...
        assert not有任何异常输出
        # 原来测试数据保留所有好的数据
        assert not result[data['signal1'].isna()[:120]).any()
        assert not result['signal1'][120:].isna().any()  # 120到row都被标记为NaN
        
        # 插取未切片部分进行插值
        assert (result['signal1'][:100] == result['signal1'][:100])
        assert (result['signal1'][120:] == result['signal1'][120:])  # 120及以后都是插值
        assert result['signal2'][120:] == result['signal2'][120:]  # 120及以后插值
        
        # 检查原始数据状态
        assert not data.iloc[120:].isna().any() # 120点没有NaN，原始数据都是好的
        assert not result.iloc[120:].isna().any()  # 120点后都是NaN，说明切片处理成功
...
def test_with_timeout_30_s():
    ...
    filter = FilterFactory.create('high_pass', 
                                      FilterConfig(multiplier=2.0, tss=600))
