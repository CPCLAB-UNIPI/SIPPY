...
def system_identification(y,u,id_method,centering='None',IC='None',\
    ...
    if ulength!=ylength:
        ...
        print("Warning! y and u lengths are not the same. The minor value between the two lengths has been chosen. The perfomed indentification may be not correct, be sure to check your input and output data alignement") ; sys.stdout.write(" ")
    ...
    if centering=='InitVal':
        ...
        sys.stdout.write("\033[0;35m")
        print("Warning! \'Centering\' argument is not valid, its value has been reset to \'None\'") ; sys.stdout.write(" ")

    if (IC=='AIC' or IC=='AICc' or IC=='BIC')==False:
        if IC != 'None':
            sys.stdout.write("\033[0;35m")
            print("Warning, no correct information criterion selected, its value has been reset to \'None\'") ; sys.stdout.write(" ")
        if id_method=='ARX':
            if len(ARX_orders)!=3:
            ...
            if (type(ARX_orders[0])==list and type(ARX_orders[1])==list and type(ARX_orders[2])==list):
                ...
                sys.exit("Error! ARX_orders must be a list containing three lists or three integers")
                identified_system='None'
...
#                identified_system='None'
        ...
        if (id_method=='ARX' or id_method=='ARMAX'):
            if (ydim!=1 or udim!=1):
                sys.exit("Error! Information criteria are implemented in SISO case for ARX and ARMAX model sets.  Use subspace methods instead for MIMO cases")
#                identified_system='None'
        if id_method=='ARX':
            from lib import arx
